"""Property-based tests for ducklake-pandas using Hypothesis.
...
def _make_int_df(n_rows: int, col_name: str, values: list[int]) -> pd.DataFrame:
    """Build a single-column Int32 DataFrame."""
    return pd.DataFrame({col_name: pd.array(values, dtype="int32")})
...
def _make_mixed_df(
    ...
    return pd.DataFrame(
        {
            "icol": pd.array(int_vals, dtype="int32"),
            "scol": str_vals,
            "fcol": np.array(float_vals, dtype="float64"),
            "bcol": bool_vals,
        }
    )
...
class TestRoundtripIntegrity:
    ...
    def test_int_roundtrip(self, n_rows: int, data):
        ...
        with _TmpCatalog() as cat:
            ...
            result = result.sort_values("val").reset_index(drop=True)
            expected = df.sort_values("val").reset_index(drop=True)
    ...
    def test_mixed_roundtrip(self, n_rows: int, data):
        ...
        with _TmpCatalog() as cat:
            ...
            expected = df.sort_values(sort_cols).reset_index(drop=True)
            assert_frame_equal(result, expected, check_dtype=False)
    ...
    def test_bool_roundtrip(self, n_rows: int, data):
        ...
        with _TmpCatalog() as cat:
            ...
            assert sorted(result["val"].tolist()) == sorted(df["val"].tolist())
...
class TestAppendConsistency:
    ...
    def test_append_equals_single_write(self, n_rows: int, data):
        ...
        df = _make_int_df(n_rows, "val", values)
        split = n_rows // 2
        df1 = df.head(split).copy()
        df2 = df.tail(n_rows - split).copy()
        ...
        single = single.sort_values("val").reset_index(drop=True)
        appended = appended.sort_values("val").reset_index(drop=True)
        assert_frame_equal(single, appended, check_dtype=False)
    ...
    def test_delete_removes_matching_rows(self, n_rows: int, threshold: int, data):
        ...
        with _TmpCatalog() as cat:
            ...
            delete_ducklake(
                cat.metadata,
                "t",
                lambda frame: frame["val"] > threshold,
                data_path=cat.data,
            )
            result = read_ducklake(cat.metadata, "t", data_path=cat.data)

            # No remaining row should match the delete predicate
            assert (result["val"] > threshold).sum() == 0

            # Remaining rows should be exactly the ones <= threshold
            expected = df[df["val"] <= threshold].sort_values("val").reset_index(drop=True)
            result = result.sort_values("val").reset_index(drop=True)
            assert_frame_equal(result, expected, check_dtype=False)
...
class TestSchemaEvolutionRoundtrip:
    ...
    def test_add_column_nulls_old_rows(self, n_rows: int, new_col: str, data):
        ...
        with _TmpCatalog() as cat:
            write_ducklake(df, cat.metadata, "t", data_path=cat.data)
            # Pandas add_column takes a dtype string
            alter_ducklake_add_column(
                cat.metadata, "t", new_col, "int32", data_path=cat.data
            )

            # Insert new row with the new column populated
            new_row = pd.DataFrame(
                {
                    "val": pd.array([42], dtype="int32"),
                    new_col: pd.array([99], dtype="int32"),
                }
            )
            write_ducklake(
            ...
            old_rows = result[result["val"] != 42]
            assert old_rows[new_col].isna().all()

            # New row has the value
            new_rows = result[result["val"] == 42]
            assert new_rows[new_col].tolist() == [99]
...
class TestPartitionRoundtrip:
    ...
    def test_partitioned_roundtrip(self, data):
        ...
        df = pd.DataFrame(
            {
                "part": part_vals,
                "val": pd.array(int_vals, dtype="int32"),
            }
        )
        ...
        with _TmpCatalog() as cat:
            ...
            assert 999 in result["val"].tolist()
