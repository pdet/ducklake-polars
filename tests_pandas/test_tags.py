"""Tests for table and column tags (metadata comments) — Pandas."""
...
def test_set_and_read_table_tag(make_write_catalog):
    ...
    create_ducklake_table(
        path, "test", {"a": "int64", "b": "varchar"}, data_path=dp
    )
    ...
    assert len(tags) == 1
    assert tags["key"].iloc[0] == "comment"
    assert tags["value"].iloc[0] == "my table description"
...
def test_set_and_read_column_tag(make_write_catalog):
    ...
    assert len(tags) == 1
    assert tags["key"].iloc[0] == "comment"
    assert tags["value"].iloc[0] == "primary key"
...
def test_delete_table_tag(make_write_catalog):
    ...
    create_ducklake_table(
        path, "test", {"a": "int64"}, data_path=dp
    )

    set_ducklake_table_tag(path, "test", "comment", "hello", data_path=dp)
    catalog = DuckLakeCatalog(path, data_path=dp)
    assert len(catalog.table_tags("test")) == 1
...
def test_delete_column_tag(make_write_catalog):
    ...
    catalog = DuckLakeCatalog(path, data_path=dp)
    assert len(catalog.column_tags("test", "a")) == 1
...
def test_multiple_tags_on_table(make_write_catalog):
    ...
    tags = catalog.table_tags("test")
    assert len(tags) == 3
    tag_dict = dict(zip(tags["key"].tolist(), tags["value"].tolist()))
...
def test_overwrite_table_tag(make_write_catalog):
    ...
    tags = catalog.table_tags("test")
    assert len(tags) == 1
    assert tags["value"].iloc[0] == "second"
...
def test_duckdb_sets_comment_we_read_tag(ducklake_catalog):
    ...
    assert len(table_tags) == 1
    assert table_tags["key"].iloc[0] == "comment"
    assert table_tags["value"].iloc[0] == "duckdb description"

    col_tags = cat.column_tags("test", "a")
    assert len(col_tags) == 1
    assert col_tags["key"].iloc[0] == "comment"
    assert col_tags["value"].iloc[0] == "col a desc"
