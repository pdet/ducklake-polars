"""Tests for ducklake-pandas DDL operations (rename column, drop table,
...
class TestRenameColumn:
    ...
    def test_rename_column_duckdb_interop(self, make_write_catalog):
        """DuckDB can read after pandas renames a column."""
        cat = make_write_catalog()
        df = pd.DataFrame({"a": [1, 2], "b": ["x", "y"]})
        write_ducklake(df, cat.metadata_path, "test", mode="error")
        ...
        assert pdf["c"].tolist() == ["x", "y"]

    def test_duckdb_rename_column_pandas_reads(self, make_write_catalog):
        """DuckDB renames a column, pandas reads correctly."""
        cat = make_write_catalog()
    ...
    def test_rename_column_multiple(self, make_write_catalog):
        ...
        result = read_ducklake(cat.metadata_path, "test")
        assert list(result.columns) == ["a", "bb", "cc"]
        assert result["a"].tolist() == [1]
        assert result["bb"].tolist() == [2]
        assert result["cc"].tolist() == [3]
...
class TestDropTable:
    ...
    def test_duckdb_drop_table_pandas_reads(self, make_write_catalog):
        """DuckDB drops a table, pandas confirms it's gone."""
        cat = make_write_catalog()
    ...
    def test_drop_and_recreate_table(self, make_write_catalog):
        ...
        df2 = pd.DataFrame({"x": ["hello", "world"]})
        write_ducklake(df2, cat.metadata_path, "test", mode="error")

        result = read_ducklake(cat.metadata_path, "test").sort_values("x").reset_index(drop=True)
...
class TestCreateSchema:
    ...
    def test_create_schema_then_create_table(self, make_write_catalog):
        ...
        result = read_ducklake(cat.metadata_path, "test", schema="myschema").sort_values("a").reset_index(drop=True)
        assert result["a"].tolist() == [1, 2, 3]

    def test_create_schema_duckdb_interop(self, make_write_catalog):
        """DuckDB can see a schema created by pandas."""
        cat = make_write_catalog()
        create_ducklake_schema(cat.metadata_path, "myschema")
    ...
    def test_duckdb_create_schema_pandas_uses(self, make_write_catalog):
        """DuckDB creates a schema, pandas creates a table in it."""
        cat = make_write_catalog()
...
class TestDropSchema:
    ...
    def test_duckdb_drop_schema_pandas_confirms(self, make_write_catalog):
        """DuckDB drops a schema, pandas can no longer use it."""
        cat = make_write_catalog()
...
class TestRenameTable:
    ...
    def test_rename_table_basic(self, make_write_catalog):
        ...
        result = read_ducklake(cat.metadata_path, "t2").sort_values("a").reset_index(drop=True)
        assert result["a"].tolist() == [1, 2, 3]
    ...
    def test_rename_table_duckdb_interop(self, make_write_catalog):
        """DuckDB can read a table renamed by pandas."""
        cat = make_write_catalog()
        df = pd.DataFrame({"a": [1, 2]})
        write_ducklake(df, cat.metadata_path, "t1", mode="error")
        ...
        pdf = cat.read_with_duckdb("t2").sort_values("a").reset_index(drop=True)
        assert pdf["a"].tolist() == [1, 2]
    ...
    def test_duckdb_rename_table_pandas_reads(self, make_write_catalog):
        """DuckDB renames a table, pandas reads by new name."""
        cat = make_write_catalog()
    ...
    def test_rename_table_then_create_with_old_name(self, make_write_catalog):
        ...
        df2 = pd.DataFrame({"x": ["hello"]})
        write_ducklake(df2, cat.metadata_path, "t1", mode="error")

        # Both tables exist
        r1 = read_ducklake(cat.metadata_path, "t1")
        assert list(r1.columns) == ["x"]
        assert r1["x"].tolist() == ["hello"]

        r2 = read_ducklake(cat.metadata_path, "t2").sort_values("a").reset_index(drop=True)
        assert r2["a"].tolist() == [1, 2]
